...
def AuthenticateFace():
    ...
    if path.exists(names_file):
        with open(names_file, 'r') as f:
            names_data = json.load(f)
    else:
        names_data = {}

    # Map the IDs to the loaded names
    names = ['']  # The first element is empty to match the 1-based ID system
    for i in range(1, len(names_data) + 1):
        names.append(names_data.get(str(i), "Unknown"))
